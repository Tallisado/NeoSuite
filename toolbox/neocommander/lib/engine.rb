...
class TaskChain
	...
	def apply_operational()
		# Gather output
		puts "result_from_last_test: "+ @taskchain_array[@current_chain].result
...
class RubySuite
  ...
  def initialize()
    ...
    #@CONFIG = read_yaml_file(@suite_root+"/env.yaml")
		task_hash = read_yaml_file(@suite_root+"/tasklist.yaml")
		@CONFIG = read_yaml_file(@suite_root+"/env.yaml")
		@COMMON = YAML::load(File.read(@suite_root+"/common.yaml"))
