...
task :run do
	Dir.chdir(@suite_home) do
		Rake::Task["runall"].reenable
		Rake::Task["runall"].invoke
	end
end
task :runall do
...
class BaseTask
	attr_accessor :taskname, :exit_status
...
class WRTest < BaseTask
	...
	def execute_cmd	
		...
		ENV['WR_DEBUG'] = 'on'
		puts "BREAK IT OFF"
		#rakefile_fullpath = toolpath("webrobot", @task_data['toolbox_tools'], @task_data['tool_path_lookup'])+"/Rakefile"
		#FileUtils.cp(rakefile_fullpath, File.dirname(__FILE__)+"/webrobot.rake")
		
		#pwd = Dir.pwd
		#Dir.chdir(toolpath("webrobot", @task_data['toolbox_tools'], @task_data['tool_path_lookup']))
		app = Rake.application.init
		...
		if @keepstdout 
			...
			Rake.application[@cmd].invoke()
			#Rake.application.options.show_tasks = :tasks
			#Rake::application.options.show_task_pattern = //
			puts "ASDASD"
			
			#puts Rake.application.display_tasks_and_comments()
			puts "ASDASD"
